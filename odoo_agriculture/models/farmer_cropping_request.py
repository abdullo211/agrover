...
class FarmerCroppingRequest(models.Model):
	...
	def view_task_request(self):
		action = self.env.ref('odoo_agriculture.action_view_farmer_cropping_task').read()[0]
	...
	def view_animal_request(self):
		action = self.env.ref('odoo_agriculture.action_crops_animals').read()[0]
		animals = []
		for rec in self:
			for crop in rec.crop_ids:
				for crop_temp in crop.crop_task_ids:
	...
	def view_fleet_request(self):
		action = self.env.ref('odoo_agriculture.action_crops_fleet_act').read()[0]
		fleets = []
		for rec in self:
			for crop in rec.crop_ids:
				for crop_temp in crop.crop_task_ids:
